...
export class AuthService {
  ...
  search(first_Name: string): Observable<User> {
    return this.http.get<User>(`${this.authUrl}/register/${first_Name}`);
  }

  viewAllUsers(): Observable<User> {
    return this.http.get<User>(`${this.authUrl}/post-feed`);
  }

  resetPassword(user: User): Observable<User> {
    return this.http.put<User>(this.authUrl + "/resetPwd", user, {
      headers: environment.headers,
      withCredentials: environment.withCredentials,
    });
  }
